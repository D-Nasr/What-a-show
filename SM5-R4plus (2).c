...
int main()
{
    int order_num = 0, max_order_num = 5;
    unsigned short quantity, index, feedback,
        available_quantity1 = 30, available_quantity2 = 20, available_quantity3 = 10;
    float net_total_price, initial_total_price, tax, total_price, discount, PRICE, tickets;
    ...
    else if (index == 2)
    {
        ...
        else if (quantity > 0 && quantity <= available_quantity2)
        {
    ...
    else if (index == 3)
    {
        ...
        else if (quantity > 0 && quantity <= available_quantity3)
        {
    ...
    /*cheking two conditions at a time : if the custumer wants to reorder and if they reached the ordering limit*/

    while (feedback == 1 && order_num != max_order_num)
    {
        order_num = order_num + 1;
        printf("Choosing from the list above, keep in mind that for: \n show 1: there's %hu available tickets\n show 2: there's %hu tickets available\n show 3: there's %hu tickets available\n Please select which available show you want either type in 1, 2 or 3: \n", available_quantity1, available_quantity2, available_quantity3);
        scanf("%hu", &index);
        ...
        if (index == 1)
        {
            ...
            /*cheking the avaibility of the quantity and in case the user entered a negative integer or a 0*/
            ...
            else if (quantity > 0 && quantity <= available_quantity1)
            {
                ...
                /*incrementing each time the variable order_num to see if they reached the ordering limit*/
        ...
        else
        {
         ...
         /*asking to check if they want to reorder again*/

        printf("Do you want to repurchase tickets from a show again? (if yes type 1, if no type 2): \n");
        scanf("%hu", &feedback);
    }
   
     /*letting the user know in case they reached the ordering limit*/

    if (order_num == max_order_num)
        printf("Sorry, you have reached the limits of reordering,");
